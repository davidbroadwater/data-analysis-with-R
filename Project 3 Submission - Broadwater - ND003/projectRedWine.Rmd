...
```{r echo=FALSE, Multivariate_Plots_Sulphates_by_Quality_Density_Plot}
...
p3 <- ggplot(aes(x = citric.acid, fill = factor(quality)), data = wine) +
  geom_histogram(binwidth = 0.01) +
  scale_fill_discrete(name="Quality")  +
  ggtitle("Citric Acid Concentration by Quality Rating in Wine") +
  xlab("Citric Acid (g/dm^3)") +
  ylab("Number of Wines")

p4 <- ggplot(aes(x = citric.acid, color = factor(quality)), data = wine) +
  geom_density() +
  scale_color_discrete(name="Quality") +
...
p6 <- ggplot(aes(x = alcohol, color = factor(quality)), data = wine) +
  geom_density() +
  ggtitle("Density of Wine ABV by Quality") +
  scale_color_discrete(name="Quality") +
...
Alcohol had the strongest correlation with red wine quality (0.476) among all of the chemical properties measured. The lowest rated wines (with a quality of 3) all had alcohol values less than or equal to 11%, while roughly 75% of the highly rated (quality of 7 or 8) wines had alcohol values greater than 11% abv. The wines rated as a 5 were skewed heavily toward lower percentages (as clearly shown in the density plot), while the rest of the wines were more evenly distributed. With the exception of wines rated as a 5, there was a clear positive relationship between alcohol and quality. This makes sense since I'd expect a higher alcohol content would be related to a higher concentration of flavor. Lower concentrations of alcohol would likely have more of a "watery" mouthfeel in comparison and might not be perceived has being of a high quality.
